...
void processDir(string dir_name) {
    ...
    {		
        ...
        if(keyboard == 's') {
			...
			ostrinstream xlength;
			xlength << box.tl().x-box.br().x;
			ostrinstream ylength;
			xlength << box.tl().y-box.br().y;
		...
		if (key == 'q' || key == 27) {
			...
			myFile_Handler.open("../info.txt", std::ios_base::app);
			...
			while (saving) {
				...
				if (!saved) {
					...
					//compute the w and h length
					ostringstream xlength;
					xlength << box.br().x-box.tl().x;
					ostringstream ylength;
					ylength << box.br().y-box.tl().y;

					string info="dataset/"+image_name+" "+"1 "+ss1.str()+" "+ss2.str()+" "+xlength.str()+" "+ylength.str();

					//write on the txt file 
					myFile_Handler << info << endl;

					//myFile_Handler.flush();
